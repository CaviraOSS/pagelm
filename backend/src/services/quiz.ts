...
function cleanOptions(v: any) {
  let o = splitOptions(v).map((s) =>
    s.replace(/^\s*(?:[A-D]\)|[1-4]\))\s*/i, "").trim()
  );
...
function coerce(items: any): QuizItem[] {
  ...
  const out = arr.slice(0, 5).map((o: any, i: number): QuizItem => {
    ...
    const explanation =
      nstr(o?.explanation, 12, 200) ||
      "The correct option matches the main idea; others do not.";
    return {
      id: i + 1,
      question: q,
      options: opts,
...
const cacheDir = path.join(process.cwd(), "storage", "cache", "quiz");
...
const S = Annotation.Root({
  ...
  rawStrict: Annotation<string>(),
  arr: Annotation<any>(),
  norm: Annotation<QuizItem[]>(),
});

const nCache = async (s: Ctx) => {
  const k = keyOf(s.topic);
  const c = readCache(k);
...
const nGen = async (s: Ctx) => {
  ...
  const parsed = await ask(s.topic, SYS);
  if (parsed) return { ...s, arr: parsed };
  return s;
};
const nRetry = async (s: Ctx) => {
  if (s.norm?.length || Array.isArray(s.arr)) return s;
  const parsed = await ask(s.topic, SYS_STRICT);
  if (parsed) return { ...s, arr: parsed };
  return s;
};
const nNormalize = async (s: Ctx) => {
  const n = coerce(s.arr);
  return { ...s, norm: n };
};
const nValidate = async (s: Ctx) => {
  if (!validQuiz(s.norm)) throw new Error("Invalid quiz JSON from model");
  const k = keyOf(s.topic);
  writeCache(k, s.norm);
  return s;
};
...
edge("__start__", "cache");
edge("cache", "gen");
edge("gen", "retry");
edge("retry", "normalize");
edge("normalize", "validate");
edge("validate", "__end__");
